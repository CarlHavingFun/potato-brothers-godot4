...
const METRIC_BACKING_COLOR := Color(0.055, 0.063, 0.067, 0.86)
...
func _build_hierarchy() -> void:
	...
	health_bar.position = Vector2(48, 32)
	health_bar.size = Vector2(248, 10)
	...
	top_center.size = Vector2(384, 96)
	add_child(top_center)
	wave_label = _label("Wave", 22, HORIZONTAL_ALIGNMENT_CENTER)
	...
	control_hint.add_theme_stylebox_override(
		"panel",
		_flat_style(Color(0.04, 0.05, 0.05, 0.84), Color.TRANSPARENT, 0)
	)
...
func _progress_bar(node_name: String, fill_color: Color) -> ProgressBar:
	...
	bar.add_theme_stylebox_override(
		"background",
		_flat_style(Color("171a1c"), Color.TRANSPARENT, 0)
	)
	bar.add_theme_stylebox_override(
		"fill",
