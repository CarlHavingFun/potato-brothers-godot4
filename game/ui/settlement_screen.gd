...
func _ready() -> void:
	...
	build_screen("胜利" if victory else "本局结束", "到达第 %d 波" % (state.current_wave if state != null else 0))
	if state != null:
		var player := state.player()
		add_info("等级 %d · 材料 %d · 武器 %d · 物品 %d" % [player.level, player.materials, player.weapon_ids.size(), player.item_ids.size()])
