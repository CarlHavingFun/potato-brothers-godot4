...
static func create(
	...
	current_wave_materials: int = 0,
	is_endless: bool = false,
	normal_wave_count: int = 0
	...
	snapshot.endless = is_endless
	snapshot.total_waves = normal_wave_count
	if player == null:
		return snapshot
	...
	snapshot.wave_materials = maxi(current_wave_materials, 0)
