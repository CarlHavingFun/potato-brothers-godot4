...
func test_sidecar_preview_promotion_preferred_publish_and_cleanup_are_explicit_and_separate() -> void:
	...
	controller.set_active_take("edited")
	assert_str((controller.jobs["job-1"] as Dictionary)["curation"]["take"]).is_equal("edited")
	controller.save_curation()
	var preview: Dictionary = controller.preview_promotion()
	assert_str(preview["take"]).is_equal("clip_2")
	assert_str(curation.calls.back()["params"]["take"]).is_equal("edited")
	var promoted: Dictionary = controller.confirm_promotion()
	assert_str(promoted["take"]).is_equal("clip_2")
	assert_str(controller.current_take).is_equal("clip_2")
	assert_str((controller.jobs["job-1"] as Dictionary)["curation"]["take"]).is_equal("clip_2")
