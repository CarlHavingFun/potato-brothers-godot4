...
func test_release_automation_and_ci_configuration_exist() -> void:
	...
	assert_str(build_script).contains('Join-Path $projectRoot "content_packs\\characters"')
	assert_str(build_script).contains('Join-Path $projectRoot "content_packs\\weapons"')
	assert_str(build_script).contains("$independentContentArtifacts")
	assert_bool(build_script.contains("NikoRuntimeSourceRoot")).is_false()
