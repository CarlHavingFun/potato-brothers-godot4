...
func test_stress_scene_uses_the_phase_one_acceptance_load() -> void:
	...
	assert_float(script.get_script_constant_map().get("MINIMUM_AVERAGE_FPS", 0.0)).is_equal(60.0)
	assert_float(script.get_script_constant_map().get("MINIMUM_ONE_PERCENT_LOW_FPS", 0.0)).is_equal(55.0)
